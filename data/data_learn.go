...
func LearnGet(guildId, channelId, ask string) (l Learn, err error) {
	l = Learn{}
	err = Db.QueryRow("select ID, ask, guild_id, channel_id, admin_id, answer, gmt_modified, pass from [kequ5060].[dbo].[guild_learn] where guild_id = $1 and ask = $2 and channel_id = $3", guildId, ask, channelId).Scan(&l.Id, &l.Ask, &l.GuildId, &l.ChannelId, &l.AdminId, &l.Answer, &l.GmtModified, &l.Pass)
	fmt.Println(l, err)
